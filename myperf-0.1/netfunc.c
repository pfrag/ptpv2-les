 ...
 * in case info->proto == _PROTO_UDP_
 */
...
int read_data(struct cnx_info_t* info, void *data, int maxlen, int from_client) {
	...
	while (remaining > 0) {
		bytes_read = recvfrom(insock, offset, remaining, 0, sa, (socklen_t*)&len);
...
int read_data_bulk(struct cnx_info_t* info, void *data, int maxlen, int from_client) {
	int len;
	int bytes_read = 0;
	int retval = 0;
	int i = 0;
	...
	return recvfrom(insock, data, 2048, 0, sa, (socklen_t*)&len);
}
...
char *get_peer_addr(struct cnx_info_t *info, int from_client) {
	char *retval = (char*)malloc(16);
	memset(retval, 0, 16);
	if (from_client) {
		strncpy(retval, inet_ntoa(info->cliaddr.sin_addr), 16);
	}
	else {
		strncpy(retval, inet_ntoa(info->addr.sin_addr), 16);
	}
	return retval;
}
